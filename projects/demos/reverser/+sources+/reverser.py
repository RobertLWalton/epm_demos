...
# Date:		Sun Jun 21 15:58:57 EDT 2020
#
...
# The following is needed to identify line number of
# CPU timeout if there is an infinite loop.
#
import faulthandler
faulthandler.enable()
...
while True:
    ...
    if line != '' and line[0] == 'p':
        ...
        if err == 'FORMAT_ERROR':
            print ( ' ', end='' );
        if err == 'INFINITE_LOOP':
            while True:
                pass
        if err == 'INFINITE_OUTPUT':
            while True:
                print ( 'MORE' );
    if err == 'NO_OUTPUT':
        sys.exit ( 0 );
